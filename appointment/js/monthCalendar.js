...
var Month = function (divId, div_id_for_times_out) {
    ...
    this.DaysOfWeek = CalSet.daysOfWeek
    // Месяцы начиная с января
    this.Months = CalSet.monthsFullName;
...
Month.prototype.listener = function () {
    ...
    function times_show(date_id) {
        ...
        let options = { weekday: CalSet.long, year: 'numeric', month: 'long', day: 'numeric' };

        if (dt in date_time_array) {
            html += '<div class="master_times" style="" id="td_' + new Date(dt).getTime() + '"> <p>' + Common.capitalizeFirstLetter(dt.toLocaleDateString(CalSet.locale, options)) + '</p>';
    ...
    div_for_time.innerHTML = no_active_times();
...
export function monthCalendar() {
    ...
    let pre_html = '<button id="btnPrev" type="button"> < Предыдущий</button>\
                ...
                <div id="divCal"></div>\
                <div id="divTimes"></div>';
    Common.divShort.classList.add('calendar-wrapper')
    Common.divShort.innerHTML = '<h3 class="back shad rad pad margin_rlb1">Выберите дату и время</h3>' + pre_html
    //Common.divShort.classList.add('calendar-wrapper')
    //Common.divShort.innerHTML = pre_html
